...
export default function MiniSquare({ row, col }) {
    const { setSelectedCell, selectedCell, Qboard, board, isPause } =
        useBoard();
        ...
        <>
            <divD
                ...
                className={`miniBox select-none flex items-center justify-center cursor-pointer bg-slate-800 w-full h-full ${
                    isSelected().current &&
                    "bg-neutral-950 outline outline-blue-500"
                } ${
            ...
            >
                ...
                {!Qboard[row][col].pencilValue == 0 &&
                    !Qboard[row][col].default && (
                        <span
                            className={` text-green-600 value text-base md:text-2xl absolute -top-1 right-1`}
                        >
